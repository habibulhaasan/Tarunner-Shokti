import { collection, doc, updateDoc, serverTimestamp, getDocs, query, where, orderBy } from "firebase/firestore";
...
export async function recalcDonationStats(uid) {
  // where("uid", ...) matches firestore.rules' bloodDonations read condition
  // exactly — see the same note in DonationsTab.jsx.
  const q = query(
    collection(db, "profiles", uid, "bloodDonations"),
    where("uid", "==", uid),
    orderBy("date", "desc")
  );

...
import { useApprovedContributions } from "./fundContributions";
...
export function useFundLedger() {
  const { items: contributions, ready: cReady } = useApprovedContributions();
  const { items: expenses, ready: eReady } = useExpenses();

  const result = useMemo(() => {
    const credits = contributions.map((c) => ({

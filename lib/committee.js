...
function defaultSettings() {
  return { roles: [] }; // [{ id, title, order, isSignatory }]
}
...
// already added are left untouched. Seeded roles default to isSignatory:
// false — admins pick which of them sign certificates in the Committee panel.
...
export function newRoleDraft(nextOrder) {
  return { id: makeRoleId(), title: "", order: nextOrder, isSignatory: false };
}
...
// Committee members currently holding a role flagged isSignatory:true in the
// Committee admin panel, in role order, live name included. Certificates
// (and anything else that needs "who signs official documents right now")
// should read from this instead of hardcoding titles — it stays correct
// automatically as roles/holders change, no code edits needed.
export function useSignatoryMembers() {
  const { members, ready } = useCommitteeMembers();
  const signatories = useMemo(
    () => members.filter((m) => m.committeeRole?.isSignatory),
    [members]
  );
  return { signatories, ready };
}

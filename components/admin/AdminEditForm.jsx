...
function EmploymentFields({ value, onChange }) {
  ...
  const selectOffice = (e) => {
    ...
    setEmployment({ officeId, officeName: hospital?.nameBn || hospital?.name || "" });
  };
  ...
  return (
    <div>
      ...
      <div className="field" style={{ marginTop: 14 }}>
        ...
        <select
          value={draft.committeeRoleId || ""}
          onChange={(e) => setDraft((d) => ({ ...d, committeeRoleId: e.target.value || null }))}
          disabled={!committeeRolesLoaded}
        >
          <option value="">Not on the committee</option>
          {committeeRoles.map((r) => (
            <option key={r.id} value={r.id}>{r.title}</option>
          ))}
        </select>
        <p className="helper-text">
          Members with a role here show up on the separate Committee page. Manage the role list under Admin → Committee.
        </p>
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 14 }}>
        ...
        <Link href={`/certificate/${profile.id}`} target="_blank" className="btn-ghost btn" style={{ width: "auto" }}>
